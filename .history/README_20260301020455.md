...
A library for training large-scale language models on GPUs. Built on NVIDIA's [Megatron Language Model](https://github.com/NVIDIA/Megatron-LM) and augmented with techniques from [DeepSpeed](https://www.deepspeed.ai) as well as novel optimizations. Neurox AI aims to be a centralized and accessible place to gather techniques for training large-scale autoregressive language models, and accelerate research into large-scale training. Neurox AI supports a wide variety of systems and hardware, including launching via Slurm, MPI, and the IBM Job Step Manager, and has been run at scale on [AWS](https://aws.amazon.com/), [CoreWeave](https://www.coreweave.com/), [ORNL Summit](https://www.olcf.ornl.gov/summit/), [ORNL Frontier](https://www.olcf.ornl.gov/frontier/),  [LUMI](https://www.lumi-supercomputer.eu/), and others.

## Why Neurox AI?

Neurox AI leverages many of the same features and technologies as the popular Megatron-DeepSpeed library but with substantially increased usability and novel optimizations. Major features include:
...
**[5/21/2024]** We now support [RWKV](https://www.rwkv.com/) with pipeline parallelism!

**[3/21/2024]** We now support Mixture-of-Experts (MoE)

**[3/17/2024]** We now support AMD MI250X GPUs

**[3/15/2024]** We now support [Mamba](https://github.com/state-spaces/mamba) with tensor parallelism!

**[8/10/2023]** We now support checkpointing with AWS S3! Activate with the `s3_path` config option.

**[9/20/2023]** We have deprecated Flash Attention 0.x and 1.x, and migrated support to Flash Attention 2.x. We don't believe this will cause problems, but if you have a specific use-case that requires old flash support using the latest Neurox AI, please raise an issue.
...
**[3/9/2023]** We have released Neurox AI 2.0.0, an upgraded version built on the latest DeepSpeed which will be regularly synced with going forward.

## Versions

Prior to 3/9/2023, Neurox AI relied on [DeeperSpeed](https://github.com/EleutherAI/DeeperSpeed), which was based on an old version of DeepSpeed (0.3.15). In order to migrate to the latest upstream DeepSpeed version while allowing users to access the old versions, we have introduced two versioned releases for both libraries.

# Contents

- [Neurox AI](#neurox-ai)
  * [Why Neurox AI?](#why-neurox-ai)
  * [News](#news)
  * [Versions](#versions)
- [Contents](#contents)
...
- [Administrative Notes](#administrative-notes)
  * [Citing Neurox AI](#citing-neurox-ai)
...
Neurox AI and Deep(er)Speed support training on multiple different nodes and you have the option of using a variety of different launchers to orchestrate multi-node jobs.
...
If using MPI then you must specify the MPI library (DeepSpeed/Neurox AI currently supports `mvapich`, `openmpi`, `mpich`, and `impi`, though `openmpi` is the most commonly used and tested) as well as pass the `deepspeed_mpi` flag in your config file:
...
In this case, you must add a new multinode runner class to `deepspeed/launcher/multinode_runner.py` and expose it as a configuration option in Neurox AI.
...
Then you can create an [sbatch](https://slurm.schedmd.com/sbatch.html) script from which to kick off your Neurox AI job. A bare-bones sbatch script on a Slurm-based cluster with 8 GPUs per node would look like this:
...
If you prefer to run Neurox AI in a Docker container, we provide a Dockerfile and docker-compose configuration.
...
```
docker compose run neurox-ai bash
```
...
drwxr-xr-x 11 mchorse mchorse 4096 Jan  8 03:52 neurox-ai
```
...
```
docker compose exec neurox-ai bash
```
...
```
docker compose run -f containers/docker/docker-compose-dockerhub.yml neurox-ai bash
```
...
Neurox AI parameters are defined in a YAML configuration file which is passed to the deepy.py launcher. We have provided some example .yml files in [configs](./configs/), showing a diverse array of features and model sizes.
...
Neurox AI includes support for Dropless Mixture of Experts (DMoE) through the `megablocks` library. It is compatible with both existing Megatron Tensor Parallelism and DeepSpeed Pipeline Parallel setups.
...
**For most uses we recommend deploying models trained using the Neurox AI library via the Hugging Face Transformers library which is better optimized for inference.**
...
Neurox AI supports evaluation on downstream tasks through the [language model evaluation harness](https://github.com/EleutherAI/lm-evaluation-harness).
...
Neurox AI is optimized heavily for training only, and Neurox AI model checkpoints are not compatible out of the box with other deep learning libraries. To make models easily loadable and shareable with end users, and for further exporting to various other frameworks, Neurox AI supports checkpoint conversion to the [Hugging Face Transformers](https://arxiv.org/abs/1910.03771) format.

Though Neurox AI supports a number of different architectural configurations, including AliBi positional embeddings, not all of these configurations map cleanly onto the supported configurations within Hugging Face Transformers.

Neurox AI supports export of compatible models into the following architectures:
- GPTNeoXForCausalLM
- LlamaForCausalLM
- MistralForCausalLM
...
To convert a Neurox AI library checkpoint to Hugging Face-loadable format, run:
```bash
...
### Importing Models Into Neurox AI

Neurox AI supplies several utilities for converting a pretrained model checkpoint into a format that can be trained within the library.

The following models or model families can be loaded in Neurox AI:
- Llama 1
- Llama 2
- CodeLlama
- Mistral-7b-v0.1
...
To convert a Llama 1 or Llama 2 checkpoint distributed by Meta AI from its original file format (downloadable [here](https://github.com/facebookresearch/llama) or [here](https://huggingface.co/meta-llama/Llama-2-7b)) into the Neurox AI library, run
...
[Weights & Biases](https://wandb.ai/site) is a machine learning monitoring platform. To use wandb to monitor your Neurox AI experiments:
...
The Neurox AI library has been widely adopted by academic and industry researchers and ported on to many HPC systems.
...
If you have found the Neurox AI library helpful in your work, you can cite this repository as

```bibtex
@software{neurox-ai,
  title = {{Neurox AI: Large Scale Autoregressive Language Modeling in PyTorch}},
  url = {https://r8.im/weareoctabit/neurox},
  year = {2026},
  version = {1.0.0},
}
```
...
## Contributing
Neurox AI is built by the open-source AI community, and relies on our amazing contributors! Please see our
[contributing](CONTRIBUTING.md) guide for more details on our CLA, code formatting, testing,
etc.

## Licensing

This repository hosts code that is part of the Neurox AI project. Licensed under the Apache License:
