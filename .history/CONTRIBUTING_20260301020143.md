# Contributing
Neurox AI welcomes your contributions!

## Prerequisites
Neurox AI uses [pre-commit](https://pre-commit.com/) to ensure that formatting is
consistent across Neurox AI. First, ensure that `pre-commit` is installed with
...
## Testing
Neurox AI tracks two types of tests: unit tests and more costly model convergence tests.
...
### Model Tests
To execute model tests, first install Neurox AI. Next, execute the model test driver:
```bash
cd tests/model/
